...
def sign_file(input_file: str, signatureID: str, x_coordinate: int, 
               y_coordinate: int, pages: List[int] = None, output_file: str = None):
    """Sign a PDF file"""
    ...
    # Signature image
    sign_filename = os.path.join(os.path.dirname(os.path.abspath(__file__)), "static", "signature.jpg")
...
def sign_folder(**kwargs):
    ...
    # Loop through the files within the input folder
...
def parse_args():
    ...
    parser.add_argument('-p', '--pages', dest='pages', type=str,
    ...
    # Convert pages from string to list of integers
    if args['pages']:
